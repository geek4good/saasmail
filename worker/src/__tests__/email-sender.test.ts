...
describe("CloudflareSender", () => {
  ...
  it("encodes large attachments (>32KB) without stack overflow", async () => {
    const fakeBinding = {
      send: vi.fn().mockResolvedValue({ messageId: "msg-large" }),
    };
    const sender = createEmailSender({
      EMAIL: fakeBinding,
    } as unknown as CloudflareBindings);

    // 64KB of data — crosses the 32KB chunk boundary
    const largeData = new Uint8Array(64 * 1024).fill(0xab);
    ...
    expect(fakeBinding.send).toHaveBeenCalledTimes(1);
    const serialized = JSON.stringify(fakeBinding.send.mock.calls[0][0]);
    expect(serialized).toContain("large.bin");
  });
});
